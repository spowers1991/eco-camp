...
const Heading = ({children, size, underlineAnimation, hideUnderlineOnMobile, className}) => {
    ...
    }, [headingRef, size]); // Update width when size changes

    switch(size) {

        case 'h1':
            return <h1 ref={(el) => {headingRef.current = el;ref(el)}} className={`relative w-fit text-[#333] font-bebas-neue text-5xl sm:text-6xl md:text-7xl xl:text-8xl font-semiBold z-20 mb-[2rem] md:mb-6 !leading-[0.85] pb-1 xl:pb-0 tracking-[-0.5px]  ${className ? className : ""}`}>
                        {children} 
                        <div className={`${hideUnderlineOnMobile ? 'hidden' : 'block'} lg:block md:absolute h-[5px] sm:h-2 left-0 bottom-[2px] sm:bottom-[2px] md:bottom-[-5px] xl:bottom-[2px] bg-[#76b375] z-20 duration-1000 ${underlineAnimation && inView ? 'w-[103%]' : 'w-0'} ${(width < 150)  && inView? '!w-[112%]' : 'w-0'}`}>
        ...
        case 'h2':
            return <h2 ref={ref} className={`text-[#333] w-fit font-bebas-neue text-4xl sm:text-5xl md:text-6xl 2xl:text-7xl font-semiBold z-20 mb-[1.5rem] md:mb-6 ${className ? className : ""}`}>
                        {children}
                        <div className={`absolute h-[5px] sm:h-2 left-0 bottom-0 sm:bottom-[-4px] lg:bottom-0 bg-[#76b375] z-20 duration-1000 ${underlineAnimation && inView ? 'w-[103%]' : 'w-0'}`}>
